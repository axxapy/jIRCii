...
public class ScriptManager extends Feature implements ClientStateListener, RuntimeWarningWatcher {
	...
	public void hashScripts() {
		...
		Iterator si = Config.getInstance().getStringList("script.files").getList().iterator();
	...
	public void addScript(String filename) {
		...
		if (fn.exists()) {
			...
			temp.save();
			Config.getInstance().sync();    // This triggers a rehash
		} else {
	...
	public boolean removeScript(String filename) {
		...
		if (remMe != null) {
			temp.remove(remMe);
			temp.save();
			Config.getInstance().sync(); // this will cause all the script values to be rehashed...
		...
		try {
			if (lame) {
				ScriptInstance defaults =
						loader.loadScript("lame", Resources.getInstance().getResourceAsStream("lame.irc"), environment);
...
//            System.out.println("Menu script loaded in: " + (System.currentTimeMillis() - start));
		...
		Iterator i = Config.getInstance().getStringList("script.files").getList().iterator();
		while (i.hasNext()) {
	...
	private boolean internalScriptLoad(String scriptFile) {
		...
		try {
			...
			InputStream ii = Resources.getInstance().getResourceAsStream(scriptFile);
